<?php
/**
*	Conversion de IPB 2 vers Connectix Boards.
*/
...
function m_init (&$notices) {
	...
	$GLOBALS['cb_db']->query('ALTER TABLE '.$GLOBALS['cb_db']->prefix.'users ADD usr_password_s VARCHAR( 5 ) NOT NULL AFTER usr_password');
	$GLOBALS['cb_db']->query('REPLACE INTO '.$GLOBALS['cb_db']->prefix.'config (cf_field, cf_value) VALUES (\'hash_type\', \'ipb\')');
}
...
function m_u_query () {
	return
		'SELECT
			m.id AS usr_id,
			name AS usr_name,
			converge_pass_hash AS usr_password,
			converge_pass_salt AS usr_password_s,
		...
		FROM '.$_SESSION['params']['migr_prefix'].'members m
		LEFT JOIN '.$_SESSION['params']['migr_prefix'].'member_extra me ON m.id=me.id
		LEFT JOIN '.$_SESSION['params']['migr_prefix'].'members_converge mc ON m.id=mc.converge_id
		WHERE m.id > '.$_SESSION['dump_lastusr'].' AND m.id <> 0
		ORDER BY m.id
...
function m_u_process (&$d) {
	...
	$d['usr_password_s'] = escape_string($d['usr_password_s']);
...
function m_tg_query () {
	return 
		'SELECT 
			f.id AS tg_id,
			f.name AS tg_name,
			f.description AS tg_comment,
			IF(ff.parent_id=-1,ff.id,0) AS tg_fromforum,
			IF(ff.parent_id>-1,ff.id,0) AS tg_fromtopicgroup,
			f.last_id AS tg_lasttopic,
			f.topics AS tg_nbtopics,
			f.posts AS tg_nbmess,
			f.position AS tg_order
		FROM '.$_SESSION['params']['migr_prefix'].'forums f
		LEFT JOIN '.$_SESSION['params']['migr_prefix'].'forums ff ON ff.id = f.parent_id
		WHERE f.parent_id != -1
		ORDER BY f.id';
}
